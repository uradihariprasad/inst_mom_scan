...
/**
 * Determine status based on OI change
 */
function getOIStatus(changeOI: number, oi: number): "strengthening" | "weakening" | "stable" {
  if (oi === 0) return "stable";
  const changePercent = (changeOI / oi) * 100;
  if (changePercent > 5) return "strengthening";
  if (changePercent < -5) return "weakening";
  return "stable";
}
...
export function analyzeStrikeLevelSR(
  ...
  for (const strike of strikes) {
    ...
    // Get previous OI for this strike to determine status
    const prevStrikeOI = prev?.strikeOI?.get(strike.strikePrice);
    
    if (strike.strikePrice < currentPrice && strike.putOI > 0) {
      // This is a support level (Put OI below price)
      const putChangeFromPrev = prevStrikeOI 
        ? strike.putOI - prevStrikeOI.putOI 
        : strike.putChangeOI;
      ...
      supportLevels.push({
        ...
        strength: getStrengthCategory(strike.putOI, maxPutOI),
        status: getOIStatus(putChangeFromPrev, strike.putOI),
      });
    }
    
    if (strike.strikePrice > currentPrice && strike.callOI > 0) {
      // This is a resistance level (Call OI above price)
      const callChangeFromPrev = prevStrikeOI 
        ? strike.callOI - prevStrikeOI.callOI 
        : strike.callChangeOI;
      ...
      resistanceLevels.push({
        ...
        strength: getStrengthCategory(strike.callOI, maxCallOI),
        status: getOIStatus(callChangeFromPrev, strike.callOI),
      });
    }
  }
  ...
  const breakoutPotential = immediateResistance?.status === "weakening" && 
    immediateResistance.distancePercent < 2;
  const breakdownPotential = immediateSupport?.status === "weakening" && 
...
export function calculateExpectedMomentum(
  ...
  if (immediateSupport?.status === "strengthening") {
    downsideProbability -= 10;
    upsideProbability += 5;
  } else if (immediateSupport?.status === "weakening") {
    downsideProbability += 15;
    upsideProbability -= 10;
  }

  // Resistance status factor  
  if (immediateResistance?.status === "weakening") {
    upsideProbability += 15;
    downsideProbability -= 10;
  } else if (immediateResistance?.status === "strengthening") {
    upsideProbability -= 10;
  ...
  if (supportResistance.pricePosition === "near_resistance") {
    ...
    if (immediateSupport?.status !== "weakening") {
      upsideProbability += 10;
      downsideProbability -= 10;
    }
  }
...
export function calculateInstitutionalScore(
  ...
  if (supportResistance?.immediateSupport) {
    if (supportResistance.immediateSupport.status === "strengthening") supportStrengthVal = 80;
    else if (supportResistance.immediateSupport.status === "weakening") supportStrengthVal = 20;
  }

  let resistanceWeakness = 50;
  if (supportResistance?.immediateResistance) {
    if (classification === "long_buildup" || classification === "short_covering") {
      if (supportResistance.immediateResistance.status === "weakening") resistanceWeakness = 80;
      else if (supportResistance.immediateResistance.status === "strengthening") resistanceWeakness = 20;
    } else {
      if (supportResistance.immediateResistance.status === "strengthening") resistanceWeakness = 80;
      else if (supportResistance.immediateResistance.status === "weakening") resistanceWeakness = 20;
    }
  }
