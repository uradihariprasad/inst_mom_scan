...
export default function Home() {
  ...
  // Auto-refresh: start scanning when authenticated
  useEffect(() => {
  ...
  const getDisplayStocks = (): ScannedStock[] => {
    ...
    stocks.sort((a, b) => {
      ...
      switch (sortField) {
        case "score":
          cmp = a.institutionalScore.totalScore - b.institutionalScore.totalScore;
          break;
        case "volume":
          cmp = a.volumeAnalysis.currentVolume - b.volumeAnalysis.currentVolume;
          break;
        case "momentum":
          cmp = a.institutionalScore.totalScore - b.institutionalScore.totalScore;
          break;
        case "oi":
          cmp = a.oiAnalysis.currentOI - b.oiAnalysis.currentOI;
          break;
        case "changeOi":
          cmp = a.oiAnalysis.changeInOI - b.oiAnalysis.changeInOI;
          break;
        case "priceChange":
  ...
  if (isCheckingAuth) {
    return <LoadingOverlay message="Initializing scanner..." />;
  }

  if (!isAuthenticated) {
    return <LoginPage sessionId={sessionId} onLogin={handleLogin} />;
  }
  ...
  return (
    <div className="min-h-screen flex flex-col">
      ...
      <MarketStatusBar
        marketStatus={scannerState.marketStatus}
        totalScanned={scannerState.totalScanned}
        totalFiltered={scannerState.totalFiltered}
        scanError={scannerState.scanError}
        ocStats={scannerState.ocStats}
      />
          ...
          <div className="px-4 py-3 border-b border-border flex flex-wrap items-center gap-3">
            ...
            <div className="flex bg-bg-secondary rounded-lg p-1">
              {(["all", "bullish", "bearish"] as const).map((tab) => (
                ...
                >
                  {tab === "all"
                    ? `All (${scannerState.allStocks.length})`
                    : tab === "bullish"
                      ? `Bullish (${scannerState.bullishStocks.length})`
                      : `Bearish (${scannerState.bearishStocks.length})`}
                </button>
              ))}
            </div>
            ...
            <div className="flex-1 min-w-[200px]">
              <input
                ...
                className="w-full max-w-sm px-3 py-1.5 bg-bg-secondary border border-border rounded-lg text-sm text-text-primary placeholder-text-muted focus:outline-none focus:border-accent-blue"
              />
            </div>
            ...
            <div className="flex items-center gap-2">
              ...
              <select
                ...
                className="px-2 py-1.5 bg-bg-secondary border border-border rounded-lg text-sm text-text-primary focus:outline-none"
              >
              ...
              <button
                ...
                className="px-2 py-1.5 bg-bg-secondary border border-border rounded-lg text-sm text-text-secondary hover:text-text-primary"
              >
                {sortDirection === "desc" ? "↓" : "↑"}
              </button>
            </div>
            ...
            {scannerState.isScanning && displayStocks.length === 0 ? (
              ...
              <div className="flex flex-col items-center justify-center h-64 text-text-muted">
                ...
                <p className="text-sm mt-1">
                  {scannerState.scanError || "Try adjusting your filters or wait for the next scan"}
                </p>
              </div>
            ) : (
              <StockLeaderboard
